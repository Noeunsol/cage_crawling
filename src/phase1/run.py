...
from src.common.sources.board import (
    discover_dcinside_trend, discover_doctornow_trend, discover_ilbe_trend,
)
from src.common.sources.rss import discover_news_trend
from src.common.extract import ExtractorRouter
from src.common.classify import build_taxonomy_index, confidence_bucket, risk_score_of, trend_score_of
from src.common.policy import load_policies
from src.common.filtering.relevance_filter import RELEVANCE_SIGNAL_TO_LV2, decide_candidate_action, decide_filter_action
from src.common.report import build_report, export_csv, export_report, print_report
from src.common.site_registry import SiteRegistry
from src.common.storage.store import Store
from src.common.classify import build_llm, copy_llm_usage
from src.phase1.verdict import apply_trend_meta, classification_action, finalize
from src.phase1.sampling import (
...
def run_trend(
    ...
    for cand in candidates:
        ...
        apply_trend_meta(rec, cand.meta)
        clean_record(rec)
        ...
        if rec.published_at and not _published_in_window(rec.published_at, cutoff):
            ...
            finalize(store, cand, rec, save_raw_text, store_content=False)
            continue
        ...
        limit_reached = bool(matcher_llm and llm_calls_by_day[llm_day] >= llm_limit)
        match = None
        if matcher_llm and not limit_reached:
            llm_calls_by_day[llm_day] += 1
            match = matcher_llm.classify(rec, policies, valid_pairs)
        if match is None:                                # 미평가/기술 실패도 fail-closed discard
            if is_root and stats:
                stats["discard"] += 1
            if matcher_llm and not limit_reached and matcher_llm.last_usage.get("total"):
                matcher_llm._record_usage(rec)
                copy_llm_usage(rec, cand)
            rec.action = rec.filter_action = "discard"
            rec.filter_reason = (
                "llm_daily_limit_reached" if limit_reached
                else (matcher_llm.last_error if matcher_llm else "llm_disabled") or "llm_failed"
            )
            finalize(store, cand, rec, save_raw_text, store_content=False)
            continue
        copy_llm_usage(rec, cand)
        ...
        rec.action = classification_action(match, rec, settings)
