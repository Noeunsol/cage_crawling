"""사이트별 정적 parser rung — 네이버 지식인(__NEXT_DATA__), 범용 커뮤니티, dcinside.
...
from src.common.schema import ExtractedContent
from src.common.extract.base import _HANGUL, first_date, title_of
...
class CommunityStaticParser:
    """커뮤니티 게시글 본문을 generic bs4 selector로 추출 (정적). 댓글은 수집하지 않는다.
    차단·JS 사이트는 대개 None을 돌려주고, 그러면 사다리가 다음 rung으로 넘어간다.
    ponytail: generic selector. 사이트별 정밀 selector는 여기에 클래스 추가로 확장."""
    name = "site_parser_community"
    # .post-content = ilbe. 앞쪽 selector가 먼저 매칭되면 그게 쓰이므로 순서가 곧 우선순위다.
    _BODY_SEL = ("article, .post-content, .post, .write_div, .view_content, "
    ...
    def extract(self, c, site, html) -> ExtractedContent | None:
        ...
        if not body:      # 이미지뿐인 글은 본문이 없으므로 여기서 탈락시킨다
            return None
