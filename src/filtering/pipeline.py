...
def build_filter_chain(
    *, blacklist_domains, openai_client, model,
    min_korean_ratio: float | None = None, enable_openai_filter: bool = True,
) -> list[NamedFilter]:
    """실제 실행에서 쓸 필터 순서를 조립한다. 규칙 기반(싸다) → LLM(비싸다) 순서를 고정한다.

    한국 관련성 사전 필터는 OpenAI 없이 한글 비율로 판단한다 (사용자 결정, 2026-08-25).
    OpenAI 필터링(taxonomy 적합성 + 한국 관련성 재판정)은 enable_openai_filter=False로 끌 수
    있다 — 끄면 규칙 기반 필터(블랙리스트/기간/한국 관련성)만 통과해도 accepted가 된다.
    ...
    if enable_openai_filter:
        openai_prompt = load_prompt("openai_filtering")
        chain.append(("openai", lambda ctx: openai_filter.check(ctx, openai_client, openai_prompt, model)))
    return chain
