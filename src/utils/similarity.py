"""근사 중복(재게시·경미 수정) 탐지용 제목 정규화 + 유사도.

형태소 분석기 의존성을 피하려고 제목은 문자 n-gram cosine을, 본문은 단어 단위 Jaccard를 쓴다.

처음엔 본문에 64bit SimHash(해밍 거리)를 썼는데, 실제 한국어 기사(150~200자)로
...
15~20배 벌어져 훨씬 잘 갈린다. 지금 규모(수천 건)면 지문(단어 집합)끼리 직접 Jaccard를
계산해도 충분히 빠르다 — 업그레이드 트리거(2026-08-31 합의): 코퍼스가 2만 건을 넘거나,
후보 1건당 근사중복 비교(check_near_duplicate 1회 호출)가 200ms를 넘으면 그때 MinHash(고정
크기 서명, O(1) 비교)로 바꾼다. 둘 다 아직이면 이대로 둔다.
"""
